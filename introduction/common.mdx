...
To configure virtual hosted buckets, you can either set `rgw_dns_name = hn.storage.weodata.vn` in weodata.conf, or add `hn.storage.weodata.vn` to the list of `hostnames` in your zonegroup configuration. 
...
<Tip>
You can define multiple hostname directly with the `rgw_dns_name` parameter.
</Tip>
